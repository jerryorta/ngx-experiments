# {{DOMAIN_TITLE}} Design Library — Contributor Notes

## Overview

- **Purpose**: Reusable presentational components, directives, and pipes for the {{DOMAIN_TITLE}} domain — no store dependency
- **Stack**: Angular 21, `ViewEncapsulation.None`, custom `--{{PREFIX}}-*` CSS variables. Built with `@angular/cdk` utilities. No Angular Material. GSAP for sophisticated animations.
- **Consumed by**: `{{DOMAIN}}-ui`, the {{DOMAIN_TITLE}} app, and Storybook
...
- Feature-local state (if truly needed) may use a lightweight NgRx Signal Store colocated in the same directory — never import from `{{DOMAIN}}-store`
...
- Test: `npx nx run {{DOMAIN}}-design-library:test`
- Lint: `npx nx run {{DOMAIN}}-design-library:lint`
...
- [ ] No imports from `{{DOMAIN}}-store` — purely presentational
