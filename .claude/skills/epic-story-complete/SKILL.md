Wraps up a finished epic story: commit the unstaged changes (in strict conventional-commit format), then either **(PR mode)** push the branch, open a PR, and transition the Jira ticket to **In Review**, or **(epic-branch mode)** push the shared epic branch the commit landed on and transition the ticket to **Done**. The path is set by `config.mergeMode` (`"pr"` default / `"epic-branch"`). Designed as the natural session-end follow-up to `/epic-next` (which leaves changes unstaged after `/jira` finishes its in-conversation phases).
...
> **Session model:** one story per session. The conversation that ran `/epic-next <board>`, planned via `/jira`, implemented it, and ran the build/test gate is the same conversation that invokes this skill. The branch in play is the per-story `feat/<key>-<slug>` (PR mode) or the shared `config.epicBranch` (epic-branch mode) that `/epic-next` put you on.

> **Where this fits in the lifecycle:**
> - **PR mode:** `/epic-next` (pick + branch + /jira) → `/epic-story-complete` (commit + push + PR + → In Review) → reviewer approves + merges → `/post-merge-cleanup` (delete branch + → Done).
> - **Epic-branch mode:** `/epic-next` (pick + checkout epic branch + /jira) → `/epic-story-complete` (commit onto `epicBranch` + push + → Done). The epic lands on `main` via one PR at the end (epic-level `/post-merge-cleanup`).
...
- The branch is `main` or some unrelated branch — refuse and stop. This skill expects the `/epic-next` context: a `feat/<key>-<slug>` branch (PR mode) or the `config.epicBranch` (epic-branch mode).
...
1. **Explicit args first.** If the args contain a board name (`nge` / etc.) and a ticket key matching `[A-Z]+-\d+`, use them verbatim. Skip to Step 1.
...
- `config.commitScope` — used as the conventional-commit scope (e.g. `charts`, `ledger`, `uidl`)
...
1. List child stories of `config.epicKey` (cached if `/epic-next` did this earlier in the session, otherwise run `python3 ~/Dev/gigasoftware-plans/scripts/jira_to_md.py board --jql 'parent = <config.epicKey>'` → JSON `[{key,status,category,type,priority,parent,summary}]` with live status).
...
## Step 6 — Build commit message (strict conventional-commit format)

This repo has **no commitlint / husky hook** — nothing mechanically rejects a malformed subject, so this skill is the enforcer. The format below matches the repo's own history (`git log --oneline --no-merges -20`) and the gigasoftware `cz-conventional-changelog` convention it was seeded from. The subject must satisfy:
...
- **Scope:** `config.commitScope` (e.g. `charts`, `ledger`, `uidl`). Always include — never omit the parens.
- **KEY at the start of the subject** (matches repo convention — see `git log --oneline --no-merges -20`).
- **Subject length:** the entire header line must be **≤ 100 characters** (the cz `subject-max-length` default this convention inherits).
- **No trailing period.**
- **Imperative, present tense** ("delete dormant load paths", not "deleted" or "deletes").
- **Lowercase first word of the descriptive subject** (the part after `<KEY> `). The KEY itself stays in its native casing (e.g. `NGE-320`).
...
4. If `<type>(<scope>): <KEY> <summary>` exceeds 100 chars, **paraphrase** to fit — keep the load-bearing nouns, drop articles/connectors. Do not truncate with ellipsis.
...
Branch:           <current-branch>   (per-story feat/ branch | epic branch ✓)
Merge mode:       <pr | epic-branch>
Files changed:    <N> (<insertions>+ / <deletions>->)

Proposed commit (conventional-commit, <subject-length>/100 chars):
  <subject line>
...
Ticket:           → In Review
<Epic-branch mode:>
Commit onto:      <config.epicBranch>   (--no-push → commit only, no push)
Push:             git push origin <config.epicBranch>
Ticket:           → Done
<both:>
Build re-run:     yes | no   (--build → yes)
Final comment:    will post the commit/branch link back to <KEY>
...
> **Never use `--no-verify`** — any pre-commit hook must run. (This repo installs none today; the rule stands so it keeps holding the day one lands.)
...
## Step 10 — Integrate the commit

The path forks on `config.mergeMode`. **Epic-branch mode → do Step 10E, then skip Steps 11–12 (no PR, no In Review) and go to Step 12E.** PR mode → do Step 10 (push) + Step 11 (PR) + Step 12 (In Review) as written. **Both paths then run Step 12S** (close the story's sub-tasks) before Step 13.

### Step 10E — Epic-branch mode: push the epic branch

The commit just landed in the main checkout directly on `config.epicBranch` (that's where `/epic-next` put you). There's no per-story branch and nothing to merge — just publish the epic branch:

```
git push origin <config.epicBranch>
```

- **`--no-push`** → stop after the commit; print `git push origin <config.epicBranch>` for the user to run later.
- **Push rejected** (someone else advanced the epic branch) → `git pull --rebase origin <config.epicBranch>` then push; on conflict, stop and surface it (never force-push).

After the push, go to **Step 12E** (transition → Done), then **Step 12S** (close sub-tasks), then Step 13.

### Step 10 — PR mode: push the per-story branch
...
## Step 12E — Epic-branch mode: transition to Done

The story is now integrated into `config.epicBranch` and verified — there is no per-story PR to wait on, so it goes straight to **Done** (mirrors how an epic-branch story is closed by hand). Read the transitions, match **Done** by name (never guess the id), and transition:

```
mcp__claude_ai_Atlassian__getTransitionsForJiraIssue(cloudId, KEY)
mcp__claude_ai_Atlassian__transitionJiraIssue(cloudId, KEY, transition={id: <done-id>})
```

If no "Done" transition is reachable, surface the available transitions and let the user pick.

## Step 12S — Close the story's sub-tasks

**Both modes run this** (after Step 12 in PR mode, or after Step 12E in epic-branch mode), before Step 13. A decomposed story's sub-tasks (e.g. `NGE-566`–`569` under `NGE-539`) are implementation slices — their deliverable is code that is now committed, so they reach **Done** when the story's implementation is wrapped.

```
mcp__claude_ai_Atlassian__getJiraIssue(cloudId, KEY, fields=["subtasks"])
```

- **No sub-tasks** → skip this step (the common case for leaf stories).
- Process the sub-tasks **not already "Done"** in parallel — for each, read its transitions then transition it to **Done** (match by name, never guess the id):
  ```
  mcp__claude_ai_Atlassian__getTransitionsForJiraIssue(cloudId, <subtask-key>)
  mcp__claude_ai_Atlassian__transitionJiraIssue(cloudId, <subtask-key>, transition={id: <done-id>})
  ```
  If a sub-task can't reach **Done** directly (workflow gap), surface it and continue — **never fail the story wrap-up over a sub-task transition**; `/post-merge-cleanup` sweeps any stragglers → Done.

## Step 13 — Final comment on the ticket

Post a short ADF comment on the Jira ticket so the work is discoverable without scanning git history.
- **PR mode:** the comment links Jira → PR (template below).
- **Epic-branch mode:** there's no PR — comment that the story was committed onto `config.epicBranch` (heading "Committed on epic branch") with the commit `<short-sha>` + branch line; drop the PR-link line.
...
<Epic-branch mode:>
✓ Story <KEY> committed on the epic branch
Commit:   <short-sha> — <subject>
Pushed:   <config.epicBranch> ✓
Ticket:   <KEY> → Done
Comment:  posted ✓ | skipped
...
Next:
  ...
  <Epic-branch mode:>
  • Story is committed on <config.epicBranch> + Done — nothing to merge per story.
  • When the whole epic is complete, open ONE PR (<config.epicBranch> → main); after it merges run /post-merge-cleanup <config.epicBranch>.
  <both:>
...
- **Push rejected** (PR mode) → stop, do not force. Per-story branches don't tolerate concurrent writers.
- **`gh pr create` failure** (PR mode) → surface the error; commit + push already succeeded, user can retry PR creation manually.
- **Push rejected** (epic-branch mode) → the epic branch advanced; `git pull --rebase origin <epicBranch>` then push. On conflict, stop and surface it; never force-push. The commit is safe locally on the epic branch.
...
- **PR mode:** this skill is the **per-story** wrap-up to In Review; the post-merge wrap-up to Done is `/post-merge-cleanup`. **Epic-branch mode:** this skill takes the story all the way to **Done** (committed on the epic branch); `/post-merge-cleanup` runs once per epic on the final `epicBranch → main` PR.
- Commit-message format is intentionally aligned with the `commit-commands:commit` plugin (enabled in `.claude/settings.json`) and the existing repo history so `git log --oneline` stays scannable.
