...
const selectionPrompt =
  ...
  `2. getAccessibleAtlassianResources() → use the first resource's id as cloudId (kept only to pass to the downstream readiness judges, which make scoped issue-link/comment MCP reads; the board + epic-description lookups below use the gigasoftware-plans script, not the MCP).\n` +
  `3. Run (Bash): python3 ~/Dev/gigasoftware-plans/scripts/jira_to_md.py board --jql '<the JQL above>'  (single-quote the JQL — it contains double quotes) → JSON [{key,status,category,type,priority,parent,summary}] of the To-Do stories (live status from Jira; the JQL already filters to To-Do). If it returns zero rows, set noStoriesRemaining=true and stories=[] and stop.\n` +
  `4. If ordering is "epic-description": run (Bash) epicPath=$(python3 ~/Dev/gigasoftware-plans/scripts/jira_to_md.py ensure ${cfg.epicKey}) then Read $epicPath (the epic's local plan file — source of truth for ordering); parse the ordered list of story keys top→bottom, preserving wave/section grouping as the group. Then cross-reference: take the first ${BATCH} keys from that ordered list that ALSO appear in the board result. ` +
  `If ordering is "jql": take the first ${BATCH} stories from the board result directly (group = priority).\n` +
  `5. Return cloudId, noStoriesRemaining, and stories[] with key, summary, 1-based wavePosition (in the picked order), and group. Cap at ${BATCH} stories. Use only real keys present in the board result — never invent keys.`;
...
const judgePrompt = (s, cloudId) =>
  ...
  `Steps:\n` +
  `1. ToolSearch: select:mcp__claude_ai_Atlassian__getJiraIssue\n` +
  `2. Read the ticket in two parts:\n` +
  `   a. CONTENT — description + any acceptance criteria — from the LOCAL plan file, not Jira: run (Bash) path=$(python3 ~/Dev/gigasoftware-plans/scripts/jira_to_md.py ensure ${s.key}) then Read $path. This local file is the source of truth for the description/spec.\n` +
  `   b. ISSUE LINKS + COMMENTS + STATUS: getJiraIssue(cloudId, ${s.key}, fields=["status","issuelinks","comment"], responseContentFormat="markdown") — this scoped read is the source for the story's live status + issue links + comments (all tracker data the plan file does not carry). Read the issue links (especially "is blocked by" / "depends on") and recent comments.\n` +
  `3. For each "is blocked by" / "depends on" linked issue: read its status from the issuelinks payload. If a link's status is ` +
  `unclear, getJiraIssue(cloudId, <that key>, fields=["status"]) to confirm. Not-Done + not-earlier-in-this-batch ⇒ open-dependency blocker.\n\n` +
  ...
  `Set key="${s.key}" and status to the current Jira status (from the step 2b response). Be decisive; do not inflate readiness, and do not manufacture blockers that the ticket does not support.`;
