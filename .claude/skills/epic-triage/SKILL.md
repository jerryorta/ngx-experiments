...
**Available boards:** any `epic.<board>.json` under `.claude/skills/epic-next/` (`rex`, `cog`, `gw`, `vwb`).
...
```
Workflow({
  scriptPath: "/Users/gigasoftware_developer/Dev/DEV_REX_1/gigasoftware/.claude/skills/epic-triage/triage-workflow.js",
  args: {
    board:             "<board>",
...
|---|----------|--------------|-----------|------|-----------------|
| 1 | REX-xxx  | Wave 2       | 🔴 BLOCKED | high | <oneLineReason> |
| 2 | REX-yyy  | Wave 2       | 🟢 READY   | high | <oneLineReason> |

Findings (non-READY only):
  REX-xxx  [blocker · open-dependency] <detail>
  REX-xxx  [warning · missing-acceptance-criteria] <detail>
```
