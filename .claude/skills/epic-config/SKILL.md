...
**Usage:**
- `/epic-config <board>` — view and edit by board name (e.g., `rex`, `cog`, `vwb`)
- `/epic-config <EPIC-KEY>` — resolve board from epic key (e.g., `REX-107`, `EC-98`)
...
```
Available boards:
  rex   REX-169  REX Concierge Prototype
  cog   EC-98    Cognition UI Implementation
  vwb   VWB-55   VWB Stories
```

Then stop — no further steps.

**Epic key argument** (matches `[A-Z]+-\d+`, e.g. `REX-107`): Extract the key prefix (letters before the first `-`). Scan all `epic.*.json` files, match by `epicKey` prefix. The board name is the `*` portion of the filename. If no match, report: "No epic config found for prefix `<PREFIX>`. Available boards: <list>." and stop.

**Board name argument** (e.g. `rex`, `cog`): Use directly. Construct path `.claude/skills/epic-next/epic.<board>.json`. If the file does not exist, report it and list available boards, then stop.
...
> "Which fields would you like to update? Describe the changes (e.g. 'add a convention', 'change the JQL', 'update the buildCommand', 'set commitScope to concierge')."
...
Proposed changes to epic.<board>.json:
  buildCommand: null → "npm run b.app.concierge"
  commitScope:  "concierge" → "concierge-backend"
```
