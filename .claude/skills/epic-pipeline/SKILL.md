...
- `/epic-pipeline <board> --auto` — autonomous hybrid mode: auto-confirm the rote gates (run cross-actor, reseed brackets, ship) and drive the whole flow end-to-end in this session. **Hard-stops and reports** the moment anything is non-nominal (triage not 🟢 GREEN, cross-actor not a clean zero-finding PASS, a compile error, or any fix-in-place). The PR merge **always stays manual** — `--auto` never merges. Principle: automate the boring path, escalate the interesting one. (No-op on autonomous boards like `cog`/`vwb` — those already run hands-off via the runner's own `/epic-story-complete`.)

**Available boards:** `rex`, `cog`, `gw`, `vwb` (any `epic.<board>.json` under `.claude/skills/epic-next/`).

**Pipeline behaviour depends on the board's `hybridMode` flag:**

| Board → `hybridMode` | Runner does | User does after |
|---|---|---|
| `rex` → `true` (hybrid) | impl only, stops at `READY_FOR_ACTOR_TEST <scenario>` | `/concierge-cross-actor-test <scenario>` interactively, then commit + PR + merge + `/post-merge-cleanup` |
| `cog` / `gw` / `vwb` → `false` (autonomous) | impl + `/epic-story-complete` (PR opened + Jira → In Review), emits `PR_URL: <url>` | Review + merge the PR, then `/post-merge-cleanup` |
...
- `hybridMode` (default `false` if absent)
...
Workflow after the runner exits:
  <hybrid>     →  /concierge-cross-actor-test <scenario>   (interactive)
                  then commit + PR + merge + /post-merge-cleanup

  <autonomous> →  review the PR, merge it (`gh pr merge <PR> --squash --auto --delete-branch`)
...
### 3a — Resolve the domain's serve command + port + log file

The server is determined by the board's domain (`config.commitScope` / `config.app`). Look the values up in the CLAUDE.md **"Local dev servers for actor / browser testing"** table. The concierge board (`rex`) resolves to:

| Serve command | Port | Log file |
|---|---|---|
| `npm run s.app.concierge` | `4202` | `/tmp/concierge-dev-server.log` |

(Other domains: read the row matching `config.app` — e.g. Media Workbench → `npm run s.app.mw` / `4201`, Cognition → `npm run s.app.cognition` / `4204`. Always use the conventional `/tmp/<app>-dev-server.log` path so `/concierge-cross-actor-test` Phase 0 reads the same log.)
...
> 2. **Launch the server detached, stdout → the log file** so it survives your own shutdown and is tailable: `<serve-command> > <log-file> 2>&1 &` (e.g. `npm run s.app.concierge > /tmp/concierge-dev-server.log 2>&1 &`).
...
> - **Hybrid path:** `printf '%s' '<KEY> · ready for actor test — /concierge-cross-actor-test <scenario>' > /tmp/claude-notify-message.txt`
...
- **Interactive (default):** ask the user whether to run it now in this same interactive session:
  ```
  Story <KEY> is ready for cross-actor verification. Run /concierge-cross-actor-test <scenario> now?
    yes  — start the test in this session
    no   — leave the branch as-is; user will drive it later
  ```
  If yes: invoke the `concierge-cross-actor-test` skill with the printed scenario as args. On PASS, recommend `/epic-story-complete` — do not auto-run it.

- **`--auto`:** do NOT ask. Invoke `concierge-cross-actor-test` directly with the scenario **plus `--auto-confirm-bracket`** (the flag built for orchestrator contexts that own the test brokerage exclusively — it auto-runs the before/after seed brackets with no Y/Enter prompts). Then evaluate the Phase 6 outcome:
  - **Clean PASS, zero findings** — status `success`, no gaps/findings (**even non-blocking notes**), no fix-in-place, dev server stayed green throughout → **auto-ship:** invoke `/epic-story-complete --yes` (commit + push + PR + ticket → In Review, confirmation gate skipped). Surface the PR, then **stop at the merge** — `--auto` never merges (show the autonomous-path merge command for the user to run).
...
After surfacing the banner, do NOT auto-merge — the user reviews the PR manually. Show the suggested commands as a literal block (don't actually run them):
...
- The runner spawns a fresh Claude Code session per story (`claude-agent-sdk`'s `query()` with `setting_sources=["project", "user"]` + `skills="all"`). The session inherits this repo's CLAUDE.md and the full skill set.
...
- For the `rex` board (hybrid) specifically, every story carries a cross-actor scenario hint via the `READY_FOR_ACTOR_TEST <scenario>` line. If the line is missing or malformed, the runner reports the story as failed — fix the inner session's instruction before retrying.
...
- **Dev server agent (hybrid only):** the dev server is owned by a background teammate named `dev server agent` (Step 3) — not by this session directly, and not by the Python runner. It runs `npx nx reset` → the domain serve command (detached, logging to the conventional `/tmp/<app>-dev-server.log`) → a persistent build-error Monitor, and reports `✅ server up` / `✅ bundle complete` / `❌ BUILD ERROR` to the orchestrator via `SendMessage`. It launches the server detached so it survives the agent's shutdown (workspace convention: leave the dev server running across sessions). The domain → serve-command/port/log mapping lives in the CLAUDE.md "Local dev servers for actor / browser testing" table.
