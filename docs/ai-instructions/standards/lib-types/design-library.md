...
- **Build here first, share second (placement hierarchy).** A new presentational primitive lives in THIS domain's own `design-library`. Promote it to the cross-app shared lib `@nge/ui-design-library` (`libs/shared/ui-design-library`, `dlc-*`) ONLY when a second application genuinely needs it — the domain lib is the default, shared is the exception.
