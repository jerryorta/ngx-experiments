...
- **Phase:** **Wave 4 COMPLETE** (2026-07-10) — app shell (nav + lazy routing + root store wiring) + the **theme-switcher showpiece**; all 3 screens **browser-verified working** under multiple personas (Professional Light/Dark, Service Provider Dark), CDK overlays (dialog) themed via the `<body>` class. Full gate green (lint+test 8 projects · `ledger-app:build` · `storybook-app:build-storybook`). Fixed two latent chart bugs surfaced by the first real render (donut centering, Overview line-preset/heights). Ready for **Wave 5** (final polish: exhaustive 6-persona sweep + `nx run-many --all`).
...
- **Wave checklist:**
  ...
  - [ ] Wave 5 — stories complete; full lint/test/build/storybook; browser-verify 3 screens × 6 personas (mostly done in W4 — remaining: exhaustive 6-persona sweep + `nx run-many --all`)
- **Decisions log:**
