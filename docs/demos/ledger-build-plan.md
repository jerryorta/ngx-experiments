...
- **Phase:** **Wave 3 COMPLETE** (2026-07-10) — three screens (Overview/Transactions/Budgets) in `ledger-ui`, each a **thin** component + colocated component-scoped `@ngrx/signals` SignalStore consuming `LedgerFacade`. `ledger-ui` = 84 tests / 6 suites green; **full gate green** (lint+test 8 projects · `ledger-app:build` · `storybook-app:build-storybook`). Ready for **Wave 4** (app shell / routing + theme-switcher showpiece).
